...
export default function UserProfile() {
  ...
  const { updateLoading, update } = updateUserProfile();
  const { deleteCurrentUser, isLoadingDeleteRequest, isDeleteUserSuccess } =
    deleteUserProfile();
  ...
  function formInvalidCheck(getUpdateProfileForm) {
    ...
    return {
      isfirstNameInvalid:
        firstName.length < 20 && !hasNameIncludeRegex.test(firstName),
      islastNameInvalid:
        (lastName.length < 20 && !hasNameIncludeRegex.test(lastName)) ||
        lastName === firstName,
  ...
  const { isfirstNameInvalid, islastNameInvalid, isPhoneInvalid } =
    formInvalidCheck(updateProfileForm);
  ...
  function handleFormError() {
    ...
    if (!updateProfileForm.phone.trim()) {
      setUpdateProfileFormError((prev) => ({
        ...prev,
        phone: "Phone shouldn't be empty",
      }));
  ...
  const isFormFilledWithoutError =
    updateProfileForm.firstName &&
    !isfirstNameInvalid &&
    updateProfileForm.lastName &&
    !islastNameInvalid &&
    updateProfileForm.phone &&
    !isPhoneInvalid;
  const hasChangesInForm =
    (userDetails.firstName !== updateProfileForm.firstName.trim() &&
      !isfirstNameInvalid) ||
    (userDetails.lastName !== updateProfileForm.lastName.trim() &&
      !islastNameInvalid) ||
    (userDetails.phone !== updateProfileForm.phone && !isPhoneInvalid);

  function updateFormSubmit(event) {

    event.preventDefault();
    if (hasChangesInForm && isFormFilledWithoutError) {
      const fullName = `${updateProfileForm.firstName} ${updateProfileForm.lastName}`;
      update({
        firstName: updateProfileForm.firstName.trim(),
        lastName: updateProfileForm.lastName.trim(),
        phone: updateProfileForm.phone,
        profileUrl:`https://ui-avatars.com/api/?name=${encodeURIComponent(fullName)}&background=0D8ABC&color=fff`
      });
    }
  }

  function handleEditMode() {
    if (userDetails.role === "employee" && userDetails?.status === "pending") {
      showToast("failure", "Update profile locked by admin");
    } else {
  ...
  let inputStyle =
    "autofill-color profile-input p-2 max-[500px]:p-1 dark:bg-slate-900 dark:text-white bg-slate-200 text-black rounded-md font-sans outline-none focus:ring-2 focus:ring-indigo-300 border border-gray-500 dark:border-gray-500 hover:ring-2 hover:ring-cyan-200 w-full";
  ...
  return (
    <div className="flex justify-center min-h-screen p-2 sm:p-4">
      <div className="w-full max-w-3xl mt-4 sm:mt-8 flex flex-col gap-4">
        <div className="w-full dark:bg-gray-900 bg-slate-50 rounded-3xl flex flex-col items-center relative px-4 py-6">
          <div className="flex flex-col items-center gap-1">
            ...
            <h2 className="font-bold text-base sm:text-xl tracking-wider">
              {userDetails.username}
            </h2>

            <p className="font-mono text-xs sm:text-sm dark:text-gray-300 text-black/70">
              {userDetails.status}
            </p>
          </div>
          ...
          <button onClick={handleEditMode} className="absolute top-3 right-3">
            {isEditMode ? (
              <FaTimesCircle
                size="1.4em"
                className="dark:text-red-700 dark:hover:text-red-600 text-red-500 hover:text-red-600"
              />
            ) : (
              <FaEdit
                size="1.4em"
                className="dark:text-inidigo-500 dark:hover:text-indigo-400 text-blue-500 hover:text-blue-600"
              />
            )}
          </button>
        </div>
          ...
          <div className="w-full dark:bg-gray-900 bg-slate-50 rounded-3xl p-4 sm:p-6">
            ...
            >
              <div className="text-left w-full">
                <label className="block mb-1 dark:text-gray-200 text-black/80 text-sm sm:text-base">
                  Email
                </label>
              ...
              <div className="flex flex-col sm:flex-row gap-4">
                <div className="w-full">
                  ...
                  <input
                    ...
                    name="firstName"
                  />
                  {updateProfileFormError.firstName && (
                ...
                <div className="w-full">
                  ...
                  <input
                    ...
                    name="lastName"
                  />
                  {updateProfileFormError.lastName && (
                    <div className="text-red-500 text-sm mt-2">
                      {updateProfileFormError.lastName}
                    </div>
                  )}
              ...
              <div className="flex justify-between transition max-sm:flex-col gap-2">
                {hasChangesInForm && (
                  <button
                    ...
                    className="dark:bg-indigo-500 dark:hover:bg-indigo-400 bg-indigo-400 hover:bg-indigo-500 transition px-4 py-2 text-sm sm:text-base w-full sm:w-40 rounded-lg font-bold"
                  >
                    Save Changes
                  </button>
                )}
                <button
                  ...
                  className="bg-red-500 hover:bg-red-600 dark:bg-red-500 dark:hover:bg-red-400 transition px-4 py-2 text-sm sm:text-base w-full sm:w-40 rounded-lg font-bold"
                >
                  Delete User
                </button>
              </div>
        ...
        {isModalOpen && (
          <ModalBox
            ...
            confirmModal={handleConfirmDeleteUser}
      ...
      {isLoadingDeleteRequest && (
        <div>
          <div className="fixed inset-0 bg-black/50 z-0 backdrop-blur-sm"></div>
          <div className="fixed z-50 top-1/2 left-1/2 -tranlate-y-1/4 h-16 w-16 animate-spin rounded-full border-4 border-blue-500 border-t-transparent"></div>
        </div>
      )}
    </div>
  );
