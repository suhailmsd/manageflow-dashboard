...
export default function ThemeToggle() {
  ...
  function themeChangeSet() {
    ...
    if (newTheme) {
      //if theme value true, updates LS to dark
  ...
  useEffect(() => {
    // useEffect renders when component calls, not when click
    if (savedItem === "dark") {
  ...
  return (
    <>
      ...
      <div className="toggle-button-group relative group">
        <button onClick={themeChangeSet} className="text-2xl">
          {theme ? <FaMoon color="blue" /> : <FaSun color="orange" />}
        </button>
        <div className="hidden group-hover:block absolute bottom-4 left-4 w-28 rounded text-center bg-gray-900 text-white z-10">
