export default function uiValidation(form) {
  ...
  return {
    ...
    isPasswordInvalid:
      userFormPassword.length > 0 &&
      (userFormPassword.length < 8 ||
        userFormPassword.length > 20 ||
        !passwordRegex.test(userFormPassword)),

    isConfirmPasswordInvalid:
      userFormConfirmPassword.length > 0 &&
      userFormConfirmPassword !== userFormPassword,

    isUsernameInvalid:
      (userFormUsername.length > 0 && !usernameRegex.test(userFormUsername)) ||
      userFormUsername.length > 15,
  };
}
